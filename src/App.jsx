...
function SpaceDust({ count = 2000 }) {
  ...
  return (
    <Points ref={ref} positions={points} stride={3}>
      <PointMaterial transparent color="#ffffff" size={0.04} sizeAttenuation={true} depthWrite={false} opacity={0.3} />
    </Points>
  );
}
...
function MoviePoster({ movie, position, onSelect, isSelected }) {
  ...
  return (
    <group position={position}>
      ...
      {!isSelected && (
        <Text position={[0, -1.8, 0]} fontSize={0.15} color="white" maxWidth={2} textAlign="center">
          {movie.title}
        </Text>
      )}
    </group>
...
export default function App() {
  ...
  if (!user) return (
    <div className="h-screen w-full bg-black flex flex-col items-center justify-center text-white p-10 overflow-hidden relative">
      ...
      <div className="z-10 text-center space-y-12 max-w-lg">
        <h1 className="text-[120px] font-black italic uppercase tracking-tighter leading-none bg-gradient-to-b from-white to-white/10 bg-clip-text text-transparent">Galaxy</h1>
        <button onClick={() => signInWithPopup(auth, provider)} className="w-full bg-white text-black py-6 rounded-2xl font-black uppercase tracking-widest flex items-center justify-center gap-4 hover:scale-105 transition-all shadow-[0_0_50px_rgba(255,255,255,0.2)]">
  ...
  return (
    <div className="h-screen w-full bg-[#00050a] relative overflow-hidden text-white">
      ...
      <div className="absolute top-0 left-0 w-full p-6 md:p-10 flex flex-col md:flex-row justify-between items-center gap-6 z-40 pointer-events-none">
        <div className="flex items-center gap-4 pointer-events-auto">
          <div className="flex p-1 bg-white/5 backdrop-blur-[20px] rounded-full border border-white/20 shadow-2xl">
            ...
            <button onClick={() => setView('favs')} className={`px-8 py-3 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${view === 'favs' ? 'bg-white text-black' : 'text-white/40 hover:text-white'}`}>Favs ({Object.keys(favs).length})</button>
          </div>
          <div className="relative group">
            <button className="px-8 py-3.5 bg-white/5 backdrop-blur-[20px] border border-white/20 rounded-full text-[10px] font-black uppercase tracking-widest">
              <Filter size={12} className="inline mr-2" style={{ color: activeGenreObj.color }}/> {activeGenreObj.name}
            </button>
            <div className="absolute top-full left-0 mt-3 w-48 bg-black/90 backdrop-blur-[30px] border border-white/10 rounded-2xl hidden group-hover:block overflow-hidden">
              {GENRES.map(g => (
                <button key={g.id} onClick={() => { setGenre(g.id); setQuery(''); }} className="w-full text-left px-6 py-4 text-[10px] uppercase font-black hover:bg-white hover:text-black transition-all">
                  {g.name}
                </button>
              ))}
            </div>
        ...
        <div className="flex items-center gap-4 pointer-events-auto">
          {view === 'explore' && (
            <form onSubmit={(e) => { e.preventDefault(); loadMovies(true); }} className="relative">
              <input type="text" placeholder="SEARCH VOID..." value={query} onChange={(e) => setQuery(e.target.value)} className="bg-white/5 backdrop-blur-[20px] border border-white/20 px-12 py-3.5 rounded-full outline-none focus:border-white/50 text-[10px] font-black uppercase tracking-widest w-64 md:w-80" />
              <Search className="absolute left-5 top-1/2 -translate-y-1/2 text-white/30" size={14} />
            </form>
          )}
          <button onClick={handleSignOut} className="p-4 bg-white/5 backdrop-blur-[20px] border border-white/20 rounded-full hover:bg-red-500/20 hover:border-red-500/50 transition-all group">
        ...
        <div className="absolute inset-y-0 right-0 z-50 flex items-center p-6 md:p-12 pointer-events-none w-full md:w-auto">
          <div className="w-full md:w-[460px] p-12 rounded-[3.5rem] pointer-events-auto bg-black/40 backdrop-blur-[40px] border border-white/20 shadow-2xl animate-in slide-in-from-right duration-700">
            ...
            <div className="flex gap-4">
              ...
              }} className="flex-1 bg-white text-black py-5 rounded-full font-black uppercase text-[10px] tracking-widest flex items-center justify-center gap-3 hover:scale-105 transition-all"><PlayCircle size={18} /> Trailer</button>
              ...
              }} className={`p-5 rounded-full border transition-all ${favs[selected.id] ? 'bg-red-600 border-red-600' : 'border-white/20 hover:bg-white/10'}`}>
      ...
      {!selected && (
        <button onClick={() => loadMovies()} className="absolute bottom-12 left-1/2 -translate-x-1/2 flex items-center gap-4 px-10 py-5 bg-white/5 backdrop-blur-[20px] border border-white/20 rounded-full font-black text-[10px] uppercase tracking-widest hover:bg-white/10 transition-all z-30 pointer-events-auto">
