...
function MoviePoster({ movie, position, onSelect, isSelected }) {
  ...
  return (
    <group position={position}>
      ...
      {!isSelected && (
        <Text position={[0, -1.6, 0]} fontSize={0.14} color="white" maxWidth={1.8} textAlign="center">
          {movie.title}
        </Text>
      )}
    </group>
...
export default function App() {
  ...
  const handleSelect = (movie, pos) => {
    if (selected?.id === movie.id) {
      ...
      setSelected(movie);
      const zoomPos = pos.clone().normalize().multiplyScalar(Math.sqrt(movies.length) * 3 + 10);
      ...
      {selected && (
        <div className="absolute inset-y-0 right-0 z-50 flex items-center p-6 md:p-12 pointer-events-none w-full md:w-auto">
          <div className="w-full md:w-[450px] p-10 rounded-[3rem] pointer-events-auto bg-black/60 backdrop-blur-3xl border border-white/10 shadow-2xl animate-in slide-in-from-right duration-500">
            ...
            <div className="flex items-center gap-2 mb-6"><Star size={14} className="text-yellow-500 fill-yellow-500" /><span className="text-yellow-500 font-black text-xs">{selected.rating.toFixed(1)}</span></div>
            <h2 className="text-4xl font-black mb-6 uppercase tracking-tighter">{selected.title}</h2>
            <p className="text-white/50 mb-10 text-sm line-clamp-6 italic">{selected.overview}</p>
            <div className="flex gap-4">
              ...
              }} className="flex-1 bg-white text-black py-4 rounded-full font-black uppercase text-[10px] tracking-widest flex items-center justify-center gap-2"><PlayCircle size={16} /> Trailer</button>
              <button onClick={() => toggleFav(selected)} className={`p-4 rounded-full border transition-all ${favs[selected.id] ? 'bg-red-600 border-red-600' : 'border-white/10'}`}><Heart size={20} fill={favs[selected.id] ? "white" : "none"} /></button>
            </div>
          </div>
        </div>
      )}
